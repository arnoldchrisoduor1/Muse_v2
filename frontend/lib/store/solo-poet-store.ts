...
const getAccessToken = () => {
  // will read from the store asynchronously.
...
export const useSoloPoetStore = create<SoloPoetState>()(
  devtools(
    (set, get) => ({
      ...
      loadDraftForEditing: (draftId: string) => {
        const { drafts } = get();
        const draftToEdit = drafts.find((draft) => draft.id === draftId);
        if (draftToEdit) {
      ...
      saveDraft: async () => {
        ...
        if (!currentDraft || !currentDraft.content.trim()) {
          console.warn(
            "Cannot save draft: current draft content is empty or missing."
          );
          return;
        }
        ...
        try {
          ...
          if (isExistingDraft) {
            response = await poemsApiClient.put(
              `/${currentDraft.id}`,
              createPoemPayload
            );
          } else {
      ...
      generateAIFeedback: async () => {
        ...
        if (!currentDraft || !currentDraft.content.trim()) {
          console.warn(
            "Cannot generate feedback: current draft content is empty or missing."
          );
      ...
      loadPoems: async (userId?: string) => {
        ...
        try {
          ...
          const path = effectiveUserId
            ? `/user/${encodeURIComponent(effectiveUserId)}`
            : `/`;
          ...
          if (Array.isArray(response.data)) {
            ...
            if (
              ...
              const hasPoemKeys = ["id", "title", "content"].some(
                (k) => k in maybePoem
              );
              if (hasPoemKeys) {
                apiPoems = [maybePoem];
                ...
                console.warn(
                  "loadPoems: unrecognized response shape, returning empty array"
                );
              }
            }
          }
          ...
          if (!Array.isArray(apiPoems)) {
            console.warn(
              "loadPoems: apiPoems is not an array after normalization. Setting to empty array.",
              apiPoems
            );
            apiPoems = [];
          }
          ...
          const publishedPoems = poems.filter(
            (poem) => poem.status === PoemStatus.PUBLISHED
          );
          ...
          console.log(
            `Loaded ${poems.length} poems (${drafts.length} drafts, ${publishedPoems.length} published)`
          );
        } catch (error) {
